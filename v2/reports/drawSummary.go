...
import (
	...
	"github.com/erneap/go-models/v2/config"
	"github.com/erneap/go-models/v2/metrics"
...
func (ds *DrawSummary) Create() (*excelize.File, error) {
	...
	cursor, err = config.GetCollection(config.DB, "metrics2", "groundoutages").Find(context.TODO(),
		filter)
	if err != nil {
		return nil, err
	}
